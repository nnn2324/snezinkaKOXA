...
void MainWindow::addKochLine(QPainterPath &path, const QPointF &p1, const QPointF &p2, int depth) {
    if (depth == 0) {
        path.lineTo(p2); // глубина 0, просто рисуем прямую линию
    } else {
        // делим отрезок p1-p2 на три равные части с помощью точек p3 и p4
        ...
        // находим вершину зубца (p5) с помощью формулы поворота вектора
        QPointF p5(
            p3.x() + v.x() * cosA - v.y() * sinA,
            p3.y() + v.x() * sinA + v.y() * cosA
            );
        // рекурсивный вызов для четырех новых получившихся отрезков меньшего размера
...
void MainWindow::updateSnowflakePath() {
    ...
    //запускаем Коха для 3 сторон базовго треугольника
